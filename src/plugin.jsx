...
function Panel({invoke,props}){const demo=props?.syntheticDemo===true;const [data,setData]=useState(null),[error,setError]=useState("");const tick=useRef(0),active=useRef(true);
 ...
 const s=data?.snapshot,state=error?"failed":data?.availability??"idle",completion=s?.ordinaryCompletion;
 return <section className="sl-shell" aria-label="StudyLink runtime status" aria-busy={!data&&!error}>
  ...
  {error?<div className="fault" role="alert"><b>FAIL CLOSED</b><span>{error}</span></div>:!s?<div className="empty"><b>NO CONTROLLED STATUS</b><span>Waiting for the CLI runtime-status sink.</span></div>:<>
   <div className="target"><span>SAFE TARGET</span><strong>{s.target?.label??"WITHHELD"}</strong><small>{words(s.rail)} / {words(s.phase)}</small></div>
   <div className="progress"><div style={{width:`${completion.percentage}%`}}/><strong>{completion.percentage}%</strong><span>{completion.answered} / {completion.denominator} ordinarily answered</span></div>
